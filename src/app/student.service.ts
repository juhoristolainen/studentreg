...
export class StudentService {
  private apiUrl = 'https://studentregister.fly.dev/students'; // apin osoite
  ...
  updateStudent(student: any): Observable<Student> {
    const mytoken = JSON.parse(sessionStorage['accesstoken']);
    student.token = mytoken.token; // pistetään token bodyn mukana
    const url = `${this.apiUrl}/update/${student._id}`;
    return this.http
  ...
  delStudent(id: string): any {
    ...
    if (mytoken) {
      ...
      console.log('Ei oikeutta poistoon!');
    }
  }
}
